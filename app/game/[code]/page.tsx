...
export default function GamePage() {
  ...
  useEffect(() => {
    console.log("useEffect 체크:", { isHost, room: !!room, gameState: !!gameState, initialized });
  ...
  if (!code) return; // 👈 이 줄을 바로 아래에 추가해 주세요!
    ...
    if (gameType === "highcard") {
      const hands = {};
      players.forEach(p => { hands[p.name] = [deck.pop()]; });
      newGameState = { phase: "reveal", hands, revealed: true };
    } else if (gameType === "blackjack") {
      const hands = {};
      players.forEach(p => { hands[p.name] = [deck.pop(), deck.pop()]; });
...
function Blackjack({ gameState, playerName, players, isHost, code }) {
  ...
  function getNextTurn(newStood = stood) {
    const idx = players.findIndex(p => p.name === currentTurn);
  ...
  function isGameOver(s, b) {
    return players.every(p => s[p.name] || b[p.name]);
  }
  ...
  const scores = {};
  players.forEach(p => { scores[p.name] = calcScore(hands[p.name] || []); });
...
function Poker({ gameState, playerName, players, isHost, code }) {
  const { hands, community, phase, deck } = gameState;
  const phaseNames = { preflop:"프리플랍", flop:"플랍 (3장)", turn:"턴 (4장)", river:"리버 (5장)", result:"최종 결과" };
      ...
      {community && community.length > 0 && (
        <div style={{ marginBottom:"24px" }}>
          ...
          <div style={{ display:"flex", gap:"8px", justifyContent:"center" }}>
            {community.map((c, i) => <CardUI key={i} card={c} />)}
          </div>
        </div>
      )}
      <div style={{ display:"flex", flexWrap:"wrap", gap:"16px", justifyContent:"center", marginBottom:"24px" }}>
...
function ResultBanner({ winner, playerName }) {
...
function actionBtn(color) {
  return { padding:"12px 24px", background:color, border:"none", borderRadius:"12px", color:"white", fontSize:"15px", fontWeight:"700", cursor:"pointer" };
}